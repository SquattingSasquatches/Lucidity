...
class UniCoursesView extends Controller
{
 	function execute()
 	{
 		$this->db->query('SELECT short_name as name, subject_id AS id FROM `subjects` AS S, `uni_subjects` AS US WHERE S.id = US.subject_id AND US.uni_id = "' . $this->params['uni_id'] . '"');
